(function ($) {
    ...
    function handle_form_submission(e) {
        ...
        // Show loading message while sending the request
        swal({
        ...
        fetch("https://api.newwaypmsco.com/api/user/register/", {
            ...
            .then(response => {
                if (!response.ok) {
                    return response.json().then(data => {
                        if (data.avatar) {
                            swal({title: 'Image Error', text: data.avatar[0], icon: 'error'});
                        } else if (data.email) {
                            swal({title: 'Email Error', text: data.email[0], icon: 'error'});
                        } else if (data.domain) {
                            swal({title: 'Domain Error', text: data.domain[0], icon: 'error'});
                        } else if (data.error) {
                            swal({title: 'Error', text: data.error, icon: 'error'});
                        } else {
                            swal({title: 'Error', text: 'Something went wrong during registration.', icon: 'error'});
                        }
            ...
            .catch(error => {
                swal.close(); // Close the loading message on error
